...
robotrouter.get("/test1", async (req:Request, res:Response) => {
    res.send("test1");
});

robotrouter.get("/", async (req:Request, res:Response) => {
    try {
...
// 로봇 정보 삭제
robotrouter.post("/api/deleterobotlist", async (req: Request, res: Response) => {
    try {
        if (!fs.existsSync('RobotSettings.json')) {
            return res.status(404).send("File not found");
        }
